...
function DirectionTestPopup({ onClose }) {
  ...
  return (
    <div onClick={decline} data-testid="dir-test-popup" style={{
      ...
      <div onClick={(e) => e.stopPropagation()} style={{
        ...
        border: "2px solid var(--fg)", boxShadow: "10px 10px 0 var(--accent)", padding: 36,
      }}>
        ...
        <div className="display" style={{ fontSize: 48, marginTop: 14, lineHeight: 0.95 }}>
          НЕ ЗНАЕШЬ,<br/><span style={{ color: "var(--accent-ink)" }}>С ЧЕГО НАЧАТЬ?</span>
        </div>
        ...
        <div style={{ display: "flex", gap: 10, marginTop: 24, flexWrap: "wrap" }}>
          <button onClick={accept} className="btn-brutal" data-testid="dir-test-accept">ПРОЙТИ ТЕСТ ↗</button>
          <button onClick={decline} className="btn-ghost" data-testid="dir-test-decline">ПОЗЖЕ</button>
        </div>
      </div>
    </div>
  );
...
function App() {
  ...
  // Тест-направление: только при первом посещении, через 5 секунд
  useEffect(() => {
    if (!booted) return;
  ...
  return (
    <div className="App noise">
      ...
      <BrowserRouter>
        ...
        <Footer />
        {showDirPopup && <DirectionTestPopup onClose={() => setShowDirPopup(false)} />}
      </BrowserRouter>
    </div>
  );
}
