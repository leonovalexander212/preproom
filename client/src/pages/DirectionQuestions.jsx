    ...
    <div ref={root} style={{ position: "relative", zIndex: 2, paddingTop: 100 }}>
      <section style={{ padding: "60px 28px 40px", maxWidth: 1280, margin: "0 auto" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr auto", gap: 40, alignItems: "end" }}>
          ...
          <div style={{
            ...
            <div className="display" style={{ fontSize: 44, color: "var(--fg)" }}>
      ...
      <section style={{ padding: "32px 28px 0", maxWidth: 1280, margin: "0 auto" }}>
        {filtered.slice(0, visible).map((q, i) => (
          <QuestionRow key={q.id} q={q} idx={i} onAsk={() => setAiQuestion(q)} />
        ))}
  ...
  return (
    <div className="q-row" data-testid={`question-${q.id}`} style={{
      ...
      >
        ...
        <div style={{ color: "var(--fg)", fontSize: 17, fontWeight: 600 }}>
          {q.text}
        </div>
...
function ProgressBar({ percent, color }) {
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 10, minWidth: 160 }}>
      ...
      <span className="mono" style={{ fontSize: 11, color: "var(--fg-dim)", minWidth: 36 }}>
        {percent}%
      </span>
    </div>
  );
...
function VideoAnswers({ questionId }) {
  ...
  useEffect(() => {
    api.getQuestionVideoAnswers(questionId)
      .then(setItems)
      .catch(() => setItems([]));
  }, [questionId]);
