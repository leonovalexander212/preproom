...
export default function DirectionQuestions() {
  ...
  useEffect(() => { setData(null); setError(null); api.getDirectionQuestions(slug).then(setData).catch((e) => setError(e.message)); }, [slug]);
      ...
      <section style={{ padding: "60px 28px 40px", maxWidth: 1280, margin: "0 auto" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr auto", gap: 40, alignItems: "end" }}>
          ...
          <div style={{
            ...
            <div className="display" style={{ fontSize: 44, color: "var(--fg)" }}>{counter.toLocaleString()}</div>
        ...
        <div style={{ display: "flex" }}>
          {[{ k: "TECHNICAL", l: "ТЕХНИЧЕСКИЕ" }, { k: "BEHAVIORAL", l: "ПОВЕДЕНЧЕСКИЕ" }].map((t) => (
            <button key={t.k} onClick={() => setTab(t.k)} className="mono" style={{
              ...
              border: "2px solid var(--fg)", padding: "12px 22px", marginRight: -2, cursor: "pointer"
            }}>{t.l}</button>
          ))}
        </div>
        <input
          ...
          style={{ background: "var(--card)", border: "2px solid var(--fg)", padding: "10px 14px", color: "var(--fg)" }}
        />
      </section>
        ...
        <div style={{ padding: "0 24px 24px", display: "grid", gap: 16 }}>
          {q.answer && (
            <div style={{
              ...
              <div className="mono" style={{ fontSize: 10, letterSpacing: "0.22em", color: "var(--accent-ink)", marginBottom: 10 }}>
                // КРАТКИЙ ОТВЕТ
              </div>
    ...
    <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
      {items.map((v, i) => (
        <a key={i} href={v.youtubeUrl} target="_blank" rel="noopener noreferrer"
          className="btn-ghost" style={{ padding: "10px 14px", fontSize: 10 }}>
          ▶ {v.timecode}
        </a>
      ))}
    </div>
